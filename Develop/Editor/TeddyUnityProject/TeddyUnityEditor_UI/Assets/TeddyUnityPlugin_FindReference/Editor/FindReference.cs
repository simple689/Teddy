...
public class FindReference : EditorWindow {
    ...
    [MenuItem("Assets/TeddyTool/Find Reference", false, 1)]
    static public void show() {
        ShowProgress(0, 0, 0);
        string curPath = AssetDatabase.GetAssetPath(Selection.activeObject.GetInstanceID());

        Dictionary<string, BetterList<string>> dictTmp = new Dictionary<string, BetterList<string>>();
        ...
        dictTmp.Add("prefab", prefabList);
        dictTmp.Add("fbx", fbxList);
        dictTmp.Add("cs", scriptList);
        dictTmp.Add("texture", textureList);
        dictTmp.Add("mat", matList);
        dictTmp.Add("shader", shaderList);
        dictTmp.Add("font", fontList);
        dictTmp.Add("level", levelList);
        dictTmp.Add("anim", null);
        dictTmp.Add("animTor", null);
        ...
        if (FindReference.instance.dict != null)
            FindReference.instance.dict.Clear();
        FindReference.instance.dict = dictTmp;
